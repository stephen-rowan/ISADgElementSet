<?php
/**
 * Leeds-GATE element set Plugin
 *
 * Creates element set for Leeds-GATE based on ISAD(G) (General International Standard Archival Description) 
...
{
    private $_elementSetName = 'Leeds-GATE element set';
    ...
    public function hookAfterSaveItem($args)
    {
        ...
        if ($args['insert'] && (boolean) get_option('pbcore_element_set_add_url_as_identifier')) {
            ...
            $elementTexts = metadata($item, array('Leeds-GATE element set', 'Identifier'), array('all' => true, 'no_escape', 'no_filter'));
            if (!in_array($text, $elementTexts)) {
                ...
                static $elementId;
                if (!$elementId) {
                    $elementId = $this->_db->getTable('Element')->findByElementSetNameAndElementName('Leeds-GATE element set', 'Identifier')->id;
                }
                $sql = "
